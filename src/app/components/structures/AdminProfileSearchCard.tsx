...
export default function AdminProfileSearchCard({name, title, id, username, color_img}: Props) {
    return (
        <div className="w-full h-fit px-2 md:px-5 py-5 bg-slate-100 font-second rounded-lg flex items-center justify-between">
            ...
            <Link href={`/admin/user/${id}/content/posts`} className="flex gap-2 rounded bg-blue-200 font-semibold text-md items-center text-blue-700 px-2 py-1 flex-nowrap hover:bg-blue-700 hover:text-blue-200"><User2 /><span className="hidden md:inline">Ver Contenido</span></Link>
        </div>
    )
}
